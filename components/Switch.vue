<template>
  <label class="switch">
    ...
    <audio ref="audioElement" :src="audioSrc" />
  </label>
</template>
...
const props = defineProps({
  modelValue: Boolean,
  audioSrc: String, // Prop pour l'URL du fichier audio
});
...
watch(
  ...
  (newValue) => {
    ...
    if (checked.value) {
      ...
      audioElement.value.currentTime = 0; // Réinitialiser l'audio s'il est désactivé
    }
  }
);
...
.switch {
  ...
  .shadow {
    ...
    width: 100%;
    height: 100%;
    background: linear-gradient(transparent 70%, rgba(black, 0.8));
    background-repeat: no-repeat;
  }
}
